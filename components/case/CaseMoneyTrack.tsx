import { moneyTrack } from "@/components/ui/styles";
...
export function MoneyTrackFill({
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden rounded-full ${moneyTrack.rest} ${className}`}
    >
      {/* Lo donado en `liana`, a plena potencia: un degradado hacia blanco lo
          apagaba y dejaba la pista en el verde medio de `accent`. Contra la
          pista no llega a 3:1; el filo de papel del extremo es lo que recorta
          el tramo. Ver `moneyTrack` en components/ui/styles.ts. */}
      <span
        aria-hidden
        className={`absolute inset-y-0 left-0 border-r-2 border-paper ${moneyTrack.donated}`}
        style={{ width: `${donated}%` }}
      />
      <span
        aria-hidden
        className={`absolute inset-y-0 left-0 border-r-2 border-paper ${moneyTrack.used}`}
        style={{ width: `${used}%` }}
      />
    </div>
  );
...
export function CaseMoneyTrack({
  ...
  return (
    <div>
      ...
      {sinEmpezar ? (
        <div>
          <p className="text-[15px] leading-snug text-muted">
            <span className="font-medium text-ink">Todavía sin aportes</span>
      ...
      <ul className="mt-3 grid grid-cols-3 gap-2 text-[12px] leading-snug">
        <li>
