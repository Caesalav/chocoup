-- El muro de lo ofrecido se enciende y se apaga sin cambiar la verificación.
--
-- Hasta aquí /ofrecido publicaba toda oferta pendiente o aceptada (0012). El
-- equipo necesitaba dos gestos distintos: hablar con quien ofrece, y decidir
-- si eso sale en el muro. `on_wall` es el segundo. Por omisión sigue encendido,
-- para que lo que entre por /ofrecer se publique igual que antes.
--
-- La vista `offer_log` se recrea entera porque Postgres no deja añadir un
-- predicado a una vista. El SELECT es el de 0012; lo único nuevo es `on_wall`
-- en el WHERE. `city_offer_activity` (0018) cuenta lo mismo: gente yendo a un
-- pueblo. Si no está en el muro, el mapa no lo pinta como movimiento.

alter table public.offers
  add column if not exists on_wall boolean not null default true;

comment on column public.offers.on_wall is
  'Si sale en /ofrecido. Independiente del estado: se puede haber aceptado y aun así no publicarlo, o quitar del muro un pendiente sin negarlo.';

-- El público inserta con el valor por omisión (true). El equipo puede apagarlo
-- después, en el mismo envío, con un update. Exigirlo aquí impide que un
-- formulario público nazca ya escondido, que sería una oferta que el muro no
-- muestra y que nadie va a buscar en la bandeja.
...
-- Qué filas entran, y las cinco condiciones dicen cosas distintas:
--
...
--
--   * En el muro. `on_wall` es el interruptor del equipo: una oferta ya
--     aceptada puede no publicarse todavía, y una pendiente puede quitarse del
--     muro sin negarla. Independiente del estado.
--
